...
public class adminCommunityListController extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public adminCommunityListController() {
        super();
    }
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(condition != null) {
			count = service.selectcommunitySearchCount3(map);
		}else {
			count = service.selectCommunityCount();
		}
		...
		if(condition != null) {
			list = service.selectCommunitySearchList3(map,pi);
		}else {
			list = service.selectCommunity(pi);
		}
		...
		request.getRequestDispatcher("WEB-INF/views/admin/adminCommunityList.jsp").forward(request, response);
	}
