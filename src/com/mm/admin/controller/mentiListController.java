 ...
 */
@WebServlet("/mentiList")
public class mentiListController extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public mentiListController() {
        super();
    }
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(condition != null) {
			count = bService.selectSearchCount2(map);
		}else {
			count = service.selectMentiCount();
		}
		...
		if(condition != null) {
			list =bService.selectSearchList2(map,pi);
		}else {
			list =service.selectMenti(pi);
		}
		...
		request.getRequestDispatcher("WEB-INF/views/admin/adminmentiList.jsp").forward(request, response);
	}
